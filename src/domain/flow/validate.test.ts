...
it('returns error when there is no trigger', () => {
  ...
  expect(diags.some((d) => d.code === 'NO_TRIGGER' && d.severity === 'error')).toBe(true);
});
...
it('returns error when there is no output', () => {
  ...
  const diags = validateFlow(flow);
  expect(diags.some((d) => d.code === 'NO_OUTPUT' && d.severity === 'error')).toBe(true);
});
...
it('returns error when an edge references a missing node', () => {
  ...
  const diags = validateFlow(flow);
  expect(diags.some((d) => d.code === 'EDGE_INVALID_ENDPOINT' && d.severity === 'error')).toBe(
    true,
  );
});
...
it('returns error when there is a cycle', () => {
  const flow = baseFlow({
    ...
    edges: [
      ...
      { id: 'e2', source: 'a1', target: 't1' }, // ciclo
    ],
  });

  const diags = validateFlow(flow);
  expect(diags.some((d) => d.code === 'CYCLE_DETECTED' && d.severity === 'error')).toBe(true);
});
...
it('returns error when condition node has no outgoing edges', () => {
  ...
  const diags = validateFlow(flow);
  expect(diags.some((d) => d.code === 'CONDITION_NO_OUTPUT' && d.severity === 'error')).toBe(true);
});